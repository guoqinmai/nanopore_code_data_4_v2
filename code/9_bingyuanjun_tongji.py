...
with open(r"/ifs1/User/mgq_st/test2_hebing/code/blastn_list.txt") as dff:
    d=dff.readlines()
    for name in d:
        fpath = (r"/ifs1/User/mgq_st/test2_hebing/8_amr_blastn//%s.fastq.gz.txt"%(name.split(".")[0]))
        ...
        df.to_csv(r"/ifs1/User/mgq_st/test2_hebing/9_amr_tongji/%s.csv"%(name.split(".")[0]))
